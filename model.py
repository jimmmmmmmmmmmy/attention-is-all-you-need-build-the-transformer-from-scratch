...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    for sentence in sentences:
        test += tuple(sentence.split(),)
    tok = {}
    for i in range(0, len(specials)):
        tok[specials[i]] = i
    for i in range(0, len(test)):
        if test[i] not in tok:
            tok[test[i]] = len(specials) + i - skip
        else:
            skip += 1
    return tok
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    dval = token_to_id[unk_token]
    final = []
    sentence = sentence.split()
    for word in sentence:
        final.append(token_to_id.get(word, dval))
    return final
...
def decode_ids_to_tokens(ids, id_to_token):
    ...
    decoded = []
    for i in ids:
        decoded.append(id_to_token.get(i, ''))
    return decoded
